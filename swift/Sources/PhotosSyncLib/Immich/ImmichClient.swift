...
public final class ImmichClient: Sendable {
    ...
    /// Result of a batch favorite update operation
    public struct FavoriteUpdateResult: Sendable {
        public let success: Bool
    ...
    /// Update favorite status for multiple assets in Immich
    /// - Parameters:
    ///   - ids: Array of Immich asset IDs to update
    ///   - isFavorite: Whether to mark assets as favorites (true) or unfavorite them (false)
    ///   - batchSize: Maximum number of assets to update per API call (default 50)
    /// - Returns: Result with success/failure counts
    public func updateFavorites(ids: [String], isFavorite: Bool, batchSize: Int = 50) async -> FavoriteUpdateResult {
        guard !ids.isEmpty else {
            return FavoriteUpdateResult(success: true, successCount: 0, failureCount: 0, error: nil)
        }
        ...
        // Process in batches
        let batches = stride(from: 0, to: ids.count, by: batchSize).map { startIndex in
            let endIndex = min(startIndex + batchSize, ids.count)
            return Array(ids[startIndex..<endIndex])
        }

        for batch in batches {
            let result = await updateFavoritesBatch(ids: batch, isFavorite: isFavorite)
            if result.success {
                totalSuccessCount += batch.count
            } else {
                totalFailureCount += batch.count
                lastError = result.error
            }
        }

        let overallSuccess = totalFailureCount == 0
    ...
    /// Internal method to update a single batch of favorites
    private func updateFavoritesBatch(ids: [String], isFavorite: Bool) async -> (success: Bool, error: String?) {
        guard let url = URL(string: "\(baseURL)/api/assets") else {
