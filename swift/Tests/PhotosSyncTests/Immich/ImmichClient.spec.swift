...
struct ImmichClientSpec {
    ...
    @Test("getAllAssets includes isFavorite field from response")
    func getAllAssetsIncludesIsFavorite() async {
        let session = createMockSession()
        let client = ImmichClient(baseURL: baseURL, apiKey: apiKey, session: session)
        ...
        MockURLProtocol.requestHandler = { request in
            return mockResponse(url: request.url!, statusCode: 200, json: [
                "assets": [
                    "items": [
                        ["id": "asset-1", "deviceAssetId": "device-1", "originalFileName": "photo1.jpg", "type": "IMAGE", "isFavorite": true],
                        ["id": "asset-2", "deviceAssetId": "device-2", "originalFileName": "photo2.jpg", "type": "IMAGE", "isFavorite": false]
                    ],
                    "nextPage": NSNull()
                ]
            ])
        ...
        let assets = await client.getAllAssets(deviceId: "photos-sync")
        #expect(assets.count == 2)
        #expect(assets[0].isFavorite == true)
        #expect(assets[1].isFavorite == false)
    }
    ...
    // MARK: - updateFavorites() tests

    @Test("updateFavorites returns success on 204 response with batch of IDs")
    func updateFavoritesSuccess() async {
        let session = createMockSession()
        let client = ImmichClient(baseURL: baseURL, apiKey: apiKey, session: session)
        ...
        MockURLProtocol.requestHandler = { request in
            ...
            if let body = request.httpBody,
               let json = try? JSONSerialization.jsonObject(with: body) as? [String: Any] {
                ...
                #expect(ids?.count == 3)
                #expect(ids?.contains("asset-1") == true)
                #expect(ids?.contains("asset-2") == true)
                #expect(ids?.contains("asset-3") == true)
            }

            return mockResponse(url: request.url!, statusCode: 204)
        }

        let result = await client.updateFavorites(ids: ["asset-1", "asset-2", "asset-3"], isFavorite: true)
        #expect(result.success == true)
    ...
    @Test("updateFavorites handles empty ID array gracefully")
    func updateFavoritesEmptyArray() async {
        let session = createMockSession()
        let client = ImmichClient(baseURL: baseURL, apiKey: apiKey, session: session)
        ...
        let result = await client.updateFavorites(ids: [], isFavorite: true)
        #expect(result.success == true)
        #expect(result.successCount == 0)
        #expect(result.failureCount == 0)
    }

    @Test("updateFavorites handles API error response")
    func updateFavoritesError() async {
        let session = createMockSession()
        let client = ImmichClient(baseURL: baseURL, apiKey: apiKey, session: session)
        ...
        let result = await client.updateFavorites(ids: ["asset-1", "asset-2"], isFavorite: false)
        #expect(result.success == false)
        #expect(result.successCount == 0)
    ...
    @Test("updateFavorites batch processing respects configurable batch size")
    func updateFavoritesBatchSize() async {
        MockURLProtocol.reset()  // Clear captured requests
        let session = createMockSession()
        let client = ImmichClient(baseURL: baseURL, apiKey: apiKey, session: session)
        ...
        // Create 5 asset IDs and use batch size of 2
        let ids = ["asset-1", "asset-2", "asset-3", "asset-4", "asset-5"]
        let result = await client.updateFavorites(ids: ids, isFavorite: true, batchSize: 2)

        #expect(result.success == true)
        #expect(result.successCount == 5)
        // Verify 3 batch requests were made: batches of 2 + 2 + 1 = 5 items
